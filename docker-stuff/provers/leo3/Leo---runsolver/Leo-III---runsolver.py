...
if __name__ == "__main__":
    ...
    parser.add_argument("--image-name", default="leo3:1.7.0-runsolver", help="specify image name")
    ...
    run_Leo_III_args = f"/artifacts/CWD/benchmark {args.wall_clock_limit} {args.intent}"

    command = f"podman run -v .:/artifacts/CWD -t {args.image_name} --timestamp {runsolver_args} run_Leo-III {run_Leo_III_args}"
    if args.dry_run:
        print(command)
    else:
